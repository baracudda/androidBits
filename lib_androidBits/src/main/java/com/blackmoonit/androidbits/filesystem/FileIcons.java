...
import com.blackmoonit.androidbits.utils.BitsThumbnailUtils;
...
public class FileIcons {
	...
	public FileIcons(Activity aAct,	MIMEtypeMap aMimeMap) {
		...
		origIconSize = BitsGraphicsUtils.dipsToPixels(mAct, origIconSize);
	}
		...
		if (theFile!=null && theFile.exists() && theFile.isFile()) {
			try {
				...
				*/
				Bitmap bmOrig;
				if (aOutputX!=0 && aOutputY!=0) {
					bmOrig = BitsGraphicsUtils.getImage(mAct,theFile,aScaling,aOutputX,aOutputY,true);
				} else {
