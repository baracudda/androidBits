...
import com.blackmoonit.androidbits.utils.FifoQueue;
...
public class FileMatcher {
					...
					case OP_DUPLICATES:
						...
						if (theTerm.mMatcher!=null && aFile.isFile()) {
							...
							if (theTerm.mMatcher.matches()) {
								...
								FileListAdapterElement[] theMatches = FileListDataSource.listFiles(aFile.getParentFile(),
										new FileFilter(){
									@Override
	 ...
	 * than {@link android.os.Environment#getExternalStorageDirectory() the SD card}, add it to the list.

	 * @param aPublicFolderList - result set
	 * @param aPublicFolder - public folder to check, use the BitsFileUtils.FOLDER_X constants.
	 */
	private static void addPublicFolder(ArrayList<File> aPublicFolderList, String aPublicFolder) {
		File thePublicFolder = BitsFileUtils.getExternalPublicFolder(aPublicFolder);
	...
	public static File[] getStandardSearchFolders(File aCurrentFolder) {
		...
		if (Build.VERSION.SDK_INT>8 && theMemDevice!=null) {
			...
			addPublicFolder(theResults,BitsFileUtils.FOLDER_DOWNLOADS);

			addPublicFolder(theResults,BitsFileUtils.FOLDER_DCIM);
			addPublicFolder(theResults,BitsFileUtils.FOLDER_PICTURES);
			addPublicFolder(theResults,BitsFileUtils.FOLDER_MUSIC);

			addPublicFolder(theResults,BitsFileUtils.FOLDER_MOVIES);
			addPublicFolder(theResults,BitsFileUtils.FOLDER_PODCASTS);

			addPublicFolder(theResults,BitsFileUtils.FOLDER_RINGTONES);
			addPublicFolder(theResults,BitsFileUtils.FOLDER_ALARMS);
			addPublicFolder(theResults,BitsFileUtils.FOLDER_NOTIFICATIONS);
		}
