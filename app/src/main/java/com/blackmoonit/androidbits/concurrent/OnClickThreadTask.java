...
public class OnClickThreadTask extends ThreadInterruptable implements OnClickListener {
	...
	public interface TaskDef<T,Q> {
		 ...
		 */
	    public Q doTask(View v, T aBeforeTaskResult);
	     ...
	     */
	    public void afterTask(View v, Q aTaskResult);

	}

	protected TaskDef mTask = null;
	 ...
	 */
	public OnClickThreadTask(TaskDef aTask) {
		setTask(aTask);
		setProcessName(null);
	 ...
	 */
	public OnClickThreadTask(TaskDef aTask, String aName, Integer aPriority) {
		setTask(aTask);
	 ...
	 */
	public OnClickThreadTask setTask(TaskDef aTask) {
		mTask = aTask;
		if (mTask==null)
	...
	static public OnClickThreadTask runThisTask(TaskDef aTask, String aName) {
		return (OnClickThreadTask)new OnClickThreadTask(aTask, aName, NORM_PRIORITY).execute();
	}
